...
It lists every category label repeated anywhere on the calendar or in the backlog, every iTunes id or song repeated within 14 days, and every artist repeated within 7 days — only for pairs whose later side is still future-dated or unscheduled, so released history never shows up. The same warnings print in the terminal when `npm run dev` or `npm run build` starts. The output names future days and categories, so it is a maintainer tool, not something to paste back to a submitter verbatim.
